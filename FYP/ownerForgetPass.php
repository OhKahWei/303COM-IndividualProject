    ...
    //Define name spaces
        ...
        if(isset($_POST["submit"]))
		{
            ...
            // Create connection
                ...
                $email = $_POST['email'];
                $result = mysqli_query($conn, "SELECT * FROM owner WHERE OwnerEmail='".$email."' ");

                while($row = mysqli_fetch_assoc($result))
				{
                            $getUserEmail = $row['OwnerEmail'];
                            $getUserPassword = $row['OwnerPassword'];
                            $getUserName = $row['OwnerName'];
                }
				...
				else
				{
                    ...
                    if ($mail->Send())
					{
                        echo "<script> alert('Please check your email spam folder to retrieve your password!'); window.location=\"ownerlogin.php\";</script>";
                    }
                    else 
					{
